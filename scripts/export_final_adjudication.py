...
def main() -> int:
    gold = json.load(
        open("data/evaluation/gold_labels_k1k7.llm-adjudicated.json", encoding="utf-8")
    )
    docs = load_docs()
    ...
    group_meta = {
        "A": (
            "建议采信（两模型一致认「有依据」，建议升 accurate/partially_supported，可快速确认）",
            "A",
        ),
    ...
    lines.append("")
    lines.append(
        "> 裁决四态：`accurate`（核心+细节有原文）/ `partially_supported`（核心有依据细节未逐字）"
        "/ `unverifiable`（核心无依据）/ `hallucination`（原文反驳）。"
    )
    ...
    for gk in ("A", "B", "C"):
        ...
        for it in its:
            ...
            lines.append(
                f"- **终裁**：`{ev}`"
                f"（确认建议 / 或改 accurate·partially_supported·unverifiable·hallucination）"
            )
